...
The explanatory variables can be many possible combinations including Student, Teaching, Whites, Male, Major, Bachelors and so on. We don't know which of them are statistically significant yet. They are all categorical variables so the actual values are 0 or 1. But in the data set, each element will have a corresponding salary as the mean salary for someone with the major that year who has a value of 1 for that factor.
